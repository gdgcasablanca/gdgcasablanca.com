...
function eventComponent ({
  ...
  const heading = createElement({element: 'h3', className: 'title', innerText: title});
  ...
  const detailsListElements = detailsList.map(list => createListItem({icon: list.icon, text: list.text}));
  const details = createElement({element: 'ul', className: 'details'});
  appendChildren({parentNode: details, children: detailsListElements});

  const anchor = createElement({element: 'a', className: 'more-link', innerText: 'Check Event page'});
...
function createListItem ({icon, text}) {
  const materialIcon = createElement({element: 'i', className: 'material-icons'});
